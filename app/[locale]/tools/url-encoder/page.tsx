      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={encode} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">{t('encodeBtn')}</button>
            <button onClick={decode} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">{t('decodeBtn')}</button>
            <button onClick={() => { setInput(""); setOutput(""); setError(""); }} className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors">{tc('clear')}</button>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              {output && <button onClick={() => navigator.clipboard.writeText(output)} className="text-sm text-blue-600 hover:underline">{tc('copy')}</button>}
            </div>
