...
function md5(str: string): string {
  ...
  x[((utf8.length * 8 + 64) >>> 9 << 4) + 14] = utf8.length * 8;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <p className="text-gray-600 mb-8">Generate MD5, SHA-1, SHA-256, and SHA-512 hashes</p>

        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">Input</label>
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="Enter text here..."
            className="w-full h-32 p-4 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent mb-4" />
        ...
        {Object.keys(hashes).length > 0 && (
          <div className="space-y-3">
            {Object.entries(hashes).map(([algo, hash]) => (
              <div key={algo} className="bg-white rounded-lg shadow p-4">
                ...
                <code className="text-xs text-gray-600 break-all font-mono">{hash}</code>
              </div>
            ))}
          </div>
        )}
